...
class DataFetcher:
    ...
    def fetch_from_github(self, repo_url: str, source_path: str, output_dir: str) -> str:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if file_count == 0:
                    logger.warning(f"No files matching patterns {self.file_patterns} found in path '{source_path}'")
                else:
                ...
                logger.error(f"Git clone timed out after 300 seconds")
                raise
    ...
    def fetch_from_blob_storage(self, blob_url: str, source_path: str, output_dir: str) -> str:
        """
        ...
        try:
            ...
            account_url = '/'.join(url_parts[:3])  # https://account.blob.core.windows.net
            ...
            for blob in blobs:
                ...
                if self._matches_pattern(blob_filename):
                    ...
                    relative_path = blob.name.replace(prefix, "", 1) if prefix else blob.name
            ...
            if file_count == 0:
                logger.warning(f"No files matching patterns {self.file_patterns} found in the specified location")
            else:
                logger.info(f"Successfully fetched {file_count} files from blob storage")
...
Examples:
  ...
  python fetch_data.py --source_type blob \\
    ...
    parser.add_argument(
        "--source_url",
        help="Source URL (required for github and blob types)"
    )
    ...
    parser.add_argument(
        "--output_dir",
        required=True,
        help="Local directory to place fetched files"
    )
    ...
    try:
        ...
        logger.info(f"Data fetching completed successfully. Files available at: {result_path}")
