    ...
    public class BasicResponseTest
    {
        ...
        public async Task Copilot_Question_Response()
        {
            ...
            string? clientId = Environment.GetEnvironmentVariable("POWER_PLATFORM_CLIENT_ID") 
                ...
                ?? throw new InvalidOperationException("POWER_PLATFORM_CLIENT_ID or ARM_CLIENT_ID is required");
            string? username = Environment.GetEnvironmentVariable("POWER_PLATFORM_USERNAME") 
                ?? throw new InvalidOperationException("POWER_PLATFORM_USERNAME is required for username/password auth");
            string? password = Environment.GetEnvironmentVariable("POWER_PLATFORM_PASSWORD") 
                ?? throw new InvalidOperationException("POWER_PLATFORM_PASSWORD is required for username/password auth");
            ...
            {
                // Extract text from the activity
