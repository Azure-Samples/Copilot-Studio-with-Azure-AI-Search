...
# NSG for Power Platform subnets (primary and failover)
resource "azurerm_network_security_group" "power_platform_nsg" {
  name                = "power-platform-nsg-${random_string.name.id}"
  location            = var.primary_location
  resource_group_name = azurerm_resource_group.this.name
  tags                = var.tags

  # Allow outbound HTTPS for Power Platform services from Power Platform subnets
  security_rule {
    ...
    source_address_prefixes    = concat(var.primary_subnet_address_spaces, var.failover_subnet_address_spaces)
    destination_address_prefix = "*"
  }

  # Allow Azure Storage access from Power Platform subnets
  security_rule {
  ...
  # Allow Azure Cognitive Services access from Power Platform subnets
  security_rule {
...
# Associate Power Platform NSG with primary subnet
resource "azurerm_subnet_network_security_group_association" "primary_subnet_nsg" {
  subnet_id                 = azurerm_subnet.primary_subnet.id
  network_security_group_id = azurerm_network_security_group.power_platform_nsg.id
}

# Associate Power Platform NSG with failover subnet
